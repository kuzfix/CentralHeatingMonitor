...
uint8_t DefaultEEPROMcontent[] = {0x11, 
  ...
  };

void WriteDefaultSensorOrderToEEPROM()
{
  int i;
  for (i=0; i<sizeof(DefaultEEPROMcontent); i++)
  {
    eeprom_write_byte((uint8_t*)i, DefaultEEPROMcontent[i]);
  }
}

#include "UART0_IRQ.h"
void PrintoutSensorOrderFromEEPROM()
{
  int i,j;
  uint8_t nSensors,value;
  
  nSensors = eeprom_read_byte((uint8_t*)0);
  fprintf_P(&UART0_str,PSTR("{0x%02x, "),nSensors);
  //Read stored order
  for (i=0; i<MAXSENSORS; i++)
  {
    for (j=0;j<OW_ROMCODE_SIZE;j++)
    {
      value = eeprom_read_byte((uint8_t*)(i*OW_ROMCODE_SIZE+j+1));
      fprintf_P(&UART0_str,PSTR("0x%02x, "), value);
      _delay_ms(10);
    }      
  }
}
