...
void ILI9341_SetScrollArea(uint16_t x, uint16_t N)
{
  ILI9341_WriteCommand(ILI9341_SCRLDEF); //Vertical Scrolling Definition
  ILI9341_WriteData((ILI9341_TFTHEIGHT-x-N)>>8);
  ILI9341_WriteData((ILI9341_TFTHEIGHT-x-N));     // START
  ILI9341_WriteData(N>>8);
  ILI9341_WriteData(N);     // N of scrolling lines
  ILI9341_WriteData(x>>8);
  ILI9341_WriteData(x);     // END
}

void ILI9341_Scroll(uint16_t idx)
{
  ILI9341_WriteCommand(ILI9341_SSTART); //Vertical Scrolling Start Address
  ILI9341_WriteData(idx>>8);
  ILI9341_WriteData(idx);     // START
}
